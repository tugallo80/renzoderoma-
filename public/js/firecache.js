 ...
 *
 * Estrategia POR TIPO DE LISTENER:
 *
 * .on('value', cb)  — cache-first + refresco automático
 *   Firebase llama al callback cada vez que cambia el dato, así que mostrar
 *   caché inmediata y luego sobreescribir con el valor real es seguro.
 *
 * .once('value')    — Firebase-first con caché como fallback de timeout
 *   .once() resuelve la Promise UNA SOLA VEZ. Si resolviéramos con caché y
 *   Firebase llega después, la página nunca se actualiza → datos viejos.
 *   Por eso: esperamos Firebase hasta ONCE_TIMEOUT ms. Si no responde a
 *   tiempo, recién ahí usamos la caché (o rechazamos si tampoco hay caché).
 *   Cuando Firebase responde (incluso tarde), guardamos en caché para la
 *   próxima vez.
 */
(function() {
    ...
    const ONCE_TIMEOUT = 8000;            // ms antes de caer al fallback de caché
    ...
    function readCache(storageKey) {
        ...
        return undefined; // undefined = sin caché (null es valor válido de Firebase)
    }
    ...
    function installPatch() {
        ...
        // ── .on('value', callback) — cache-first, Firebase refresca solo ──
        proto.on = function (eventType, callback) {
            ...
            // Render inmediato desde caché (Firebase llamará de nuevo con dato fresco)
            ...
            // Actualizar caché con cada dato fresco que llega de Firebase
